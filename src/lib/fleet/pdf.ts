...
export function downloadVehicleServicePdf(service: VehicleService) {
  ...
  doc.text("Registro de servicio · Flotilla empresarial", margin, y);
  ...
  const left = [
    ...
    service.odometerKm != null ? `Odómetro: ${service.odometerKm} km` : undefined,
  ].filter(Boolean) as string[];
  const right = [
    service.workshop || undefined,
    service.technician ? `Técnico: ${service.technician}` : undefined,
