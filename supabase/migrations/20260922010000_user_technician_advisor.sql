-- Marca de técnico y asesor de servicios en la ficha del colaborador.
...
where lower(role) in ('servicio')
  and is_technician = false;

update public.app_users
set
  is_service_advisor = true,
  job_title = case
    when coalesce(trim(job_title), '') = '' then 'Asesor de servicios'
    else job_title
  end
where
  lower(username) like '%angel.ramirez%'
  or lower(replace(replace(coalesce(full_name, ''), 'á', 'a'), 'Á', 'A'))
    like '%angel%ramirez%';
...
drop function if exists public.update_app_user_profile(
...
drop function if exists public.set_app_user_service_flags(uuid, boolean, boolean);
...
create function public.set_app_user_service_flags(
  p_user_id uuid,
...
begin
  ...
  set
    is_technician = coalesce(p_is_technician, u.is_technician),
    is_service_advisor = coalesce(p_is_service_advisor, u.is_service_advisor),
    updated_at = now()
  where u.id = p_user_id
  returning
    u.id,
...
revoke all on function public.set_app_user_service_flags(uuid, boolean, boolean) from public;
grant execute on function public.set_app_user_service_flags(uuid, boolean, boolean) to anon, authenticated;
...
create function public.create_app_user(
  ...
  p_is_technician boolean default false,
  p_is_service_advisor boolean default false
)
returns table (
  id uuid,
  username text,
...
begin
  ...
  insert into public.app_users (
    ...
    is_technician, is_service_advisor
  )
  values (
    v_username,
    ...
    coalesce(p_is_technician, false),
    coalesce(p_is_service_advisor, false)
  )
  returning
    app_users.id,
    app_users.username,
...
revoke all on function public.create_app_user(
  text, text, text, text, text, text, text, text, text, text, text, date, date, text, text, text, text, text, text, boolean, boolean
) from public;
...
revoke all on function public.update_app_user_profile(
  uuid, text, text, text, text, text, text, text, text, text, text, date, date, text, text, text, text, text, text, text, boolean, boolean
) from public;
...
revoke all on function public.update_own_profile(
  uuid, text, text, text, text, text, date, text, text, text, text, text, text, boolean, boolean
) from public;
